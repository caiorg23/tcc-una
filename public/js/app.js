...
window.addEventListener('DOMContentLoaded', function () {
  ...
  buildSchedule();
  buildServicesList();
  var selectedTimeValue = (_document$getElementB = document.getElementById('selectedTime')) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.value;
  if (selectedTimeValue) {
    var selectedTimeItem = Array.from(document.querySelectorAll('.time-grid .time-item')).find(function (item) {
      return item.textContent.trim() === selectedTimeValue && !item.classList.contains('busy');
    });
    if (selectedTimeItem) {
      selectTime(selectedTimeItem);
    } else {
      var firstTime = document.querySelector('.time-grid .time-item:not(.busy)');
      if (firstTime) {
        selectTime(firstTime);
      }
    }
  } else {
    var _firstTime = document.querySelector('.time-grid .time-item:not(.busy)');
    if (_firstTime) {
      selectTime(_firstTime);
    }
  }
  var selectedServiceIdsValue = (_document$getElementB2 = document.getElementById('selectedServiceIds')) === null || _document$getElementB2 === void 0 ? void 0 : _document$getElementB2.value;
  if (selectedServiceIdsValue) {
    var serviceIds = selectedServiceIdsValue.split(',').filter(Boolean);
    serviceIds.forEach(function (id) {
      var selectedCard = document.querySelector("#scheduleServiceList .service-card[data-service-id=\"".concat(id, "\"]"));
      if (selectedCard) {
...
function buildSchedule() {
  ...
  var maxDays = 14;
  var selectedDateValue = (_document$getElementB3 = document.getElementById('selectedDate')) === null || _document$getElementB3 === void 0 ? void 0 : _document$getElementB3.value;
  if (!grid) {
    return;
  }
  ...
  var _loop = function _loop() {
    ...
    var dateValue = d.toISOString().slice(0, 10);
    var isSelected = selectedDateValue ? dateValue === selectedDateValue : i === 1;
    var item = document.createElement('div');
    item.className = "date-item".concat(isSelected ? ' selected' : '');
    item.innerHTML = "<div class=\"date-day\">".concat(days[d.getDay()], "</div><div class=\"date-num\">").concat(d.getDate(), "</div>");
    item.onclick = function () {
      document.querySelectorAll('.date-item').forEach(function (x) {
  ...
  var selectedServiceId = (_document$getElementB4 = document.getElementById('selectedServiceId')) === null || _document$getElementB4 === void 0 ? void 0 : _document$getElementB4.value;
  if (svcList && selectedServiceId) {
    var selectedCard = svcList.querySelector(".service-card[data-service-id=\"".concat(selectedServiceId, "\"]"));
    if (selectedCard) {
      selectedCard.classList.add('selected');
    }
  }
...
function buildServicesList() {
  ...
  services.forEach(function (service) {
    ...
    var rawIcon = service.icon || 'bi bi-question-circle';
    var icon = iconMap[rawIcon] || rawIcon;
    if (typeof icon !== 'string' || (!icon.startsWith('fa') && !icon.startsWith('bi'))) {
      icon = 'bi bi-question-circle';
    }
    var iconHtml = "<i class=\"".concat(icon, "\"></i>");
    var iconBg = service.bg || 'si-green';
...
function updateCount() {
  var count = document.querySelectorAll('#scheduleServiceList .service-card.selectable.selected').length;
  var label = document.getElementById('svcCountLabel');
  if (label) {
...
function selectTime(el) {
  ...
  document.querySelectorAll('.time-item').forEach(function (item) {
...
function quickReply(text) {
  ...
  var reply = botReplies[key] || 'Obrigado pela mensagem! Em breve um atendente responderá. 🙂';
  setTimeout(function () {
    return addMsg(reply, 'bot');
  }, 600);
}
function sendMsg() {
  ...
  var reply = 'Entendido! Para mais informações, use os botões rápidos abaixo ou aguarde nosso atendimento. 🙂';
  Object.keys(botReplies).forEach(function (key) {
